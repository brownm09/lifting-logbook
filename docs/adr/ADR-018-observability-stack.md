...
**Status:** Accepted
**Date:** 2026-05-08
**Closes:** [#204](https://github.com/merickvaughn/lifting-logbook/issues/204) (records the decision for tracking epic [#199](https://github.com/merickvaughn/lifting-logbook/issues/199))

---

## Context

The repo has zero observability instrumentation. There is no OpenTelemetry SDK, no metrics pipeline, no structured logging, and no alerting. A request that fails across `apps/web` → `apps/api` → Postgres cannot be correlated end-to-end, and the system has no signal that something is wrong until a user reports it. Tracking issue [#199](https://github.com/merickvaughn/lifting-logbook/issues/199) plans the full rollout across five child PRs ([#204](https://github.com/merickvaughn/lifting-logbook/issues/204)–[#208](https://github.com/merickvaughn/lifting-logbook/issues/208)); this ADR locks in the technology and topology decisions so each child PR has a fixed reference.
...
- **Unified Grafana UX.** Tempo, Loki, and Mimir are designed to cross-link inside Grafana — clicking a `trace_id` in a log line jumps to the trace; clicking a span jumps to the matching log lines. This is the workflow the on-call runbook ([#208](https://github.com/merickvaughn/lifting-logbook/issues/208)) is built around.
...
- **No SLOs yet.** Three RED alert rules ship in [#207](https://github.com/merickvaughn/lifting-logbook/issues/207) — error-rate > 1%, p95 > 1s, no-requests-10m — but proper SLOs and error budgets are explicitly deferred to [#201 (On-Call Readiness)](../proposals/2026-05-08-on-call-readiness.md). The initial alerts are coarse signals, not refined SLO violations. The `no-requests-10m` rule in particular is known to fire spuriously on low-traffic services with diurnal patterns; [#207](https://github.com/merickvaughn/lifting-logbook/issues/207) must gate it to a known-traffic window (business hours, or a `for:` clause sized to the longest expected idle period) before the rule lands in production.
- **Head-based 100% sampling.** Will not scale to high-traffic production. Tracked in [#210](https://github.com/merickvaughn/lifting-logbook/issues/210); revisit when sustained ingest crosses 1,000 spans/minute (24h avg) or monthly Grafana Cloud trace cost exceeds $25.
...
The five child PRs of [#199](https://github.com/merickvaughn/lifting-logbook/issues/199) implement this ADR:

| Child | Scope |
|---|---|
| [#204](https://github.com/merickvaughn/lifting-logbook/issues/204) | This ADR (no code). |
| [#205](https://github.com/merickvaughn/lifting-logbook/issues/205) | `apps/api`: OTel SDK bootstrap (`apps/api/src/otel.ts`), Prisma instrumentation, `nestjs-pino` with trace-context hook. |
| [#206](https://github.com/merickvaughn/lifting-logbook/issues/206) | `apps/web`: `apps/web/instrumentation.ts` via `@vercel/otel`; W3C `traceparent` propagation. |
| [#207](https://github.com/merickvaughn/lifting-logbook/issues/207) | `infra/observability/otel-collector.yaml`; docker-compose extensions; `infra/kubernetes/charts/otel-collector/`; Cloud Run sidecar template; RED dashboard JSON; three Prometheus alert rules. |
| [#208](https://github.com/merickvaughn/lifting-logbook/issues/208) | `docs/runbooks/observability.md`; README links; ROADMAP shipped row. |
...
| [Google SRE Book — Monitoring Distributed Systems](https://sre.google/sre-book/monitoring-distributed-systems/) | The RED/USE framing the three alert rules in [#207](https://github.com/merickvaughn/lifting-logbook/issues/207) are built on. |
...
| [ADR-013 — Testing strategy](ADR-013-testing-strategy.md) | The OTel smoke test in `apps/api` ([#205](https://github.com/merickvaughn/lifting-logbook/issues/205)) follows the Jest + in-memory adapter pattern from ADR-013. |
