...
- **Merge queue eligibility requires organization ownership** ([#729](https://github.com/brownm09/lifting-logbook/issues/729)) — this repo is currently owned by a personal user account and does not qualify until transferred. #694's `merge_group` wiring stays in place regardless (inert until the queue is enabled, harmless either way, and free option value if/when the transfer happens).
...
- [#729](https://github.com/brownm09/lifting-logbook/issues/729) — the organization-ownership eligibility gap discovered post-merge, and the tracked prerequisite for #695.
- [#730](https://github.com/brownm09/lifting-logbook/issues/730) — this correction.
