...
**Reviewed:** 2026-04-07
**Review outcome:** Pass
**Implementation status (2026-06-10):** Partially implemented — both GKE and Cloud Run deploy the same image, but the **90/10 Cloud Load Balancer traffic split (below) is not wired**: there are no weighted `google_compute_backend_service` backends, URL-map split rules, or Ingress canary annotations in `infra/`. Each target is reachable independently today; the A/B traffic-split mechanism is unimplemented. Surfaced by the 2026-06-08 architecture review ([#464](https://github.com/merickvaughn/lifting-logbook/issues/464)) and reconciled in [#495](https://github.com/merickvaughn/lifting-logbook/issues/495).

**Cost retirement (2026-07-18, [#860](https://github.com/merickvaughn/lifting-logbook/issues/860)):** After the $300 free-trial credit was exhausted, this GKE-vs-Cloud-Run A/B was re-evaluated for cost. Because the 90/10 traffic split above was never wired, the always-on staging Autopilot cluster (cluster fee + api/web pods + otel DaemonSet + GKE Ingress LB, ~$70–130/mo) produced no A/B payoff. **Staging GKE is now disabled** (`enable_gke = false` in `terraform.tfvars.staging`); both environments run **Cloud-Run-only**. The GKE Terraform (`gke.tf`), Helm charts, and CI GKE-deploy steps are retained behind the `enable_gke` flag and can be re-enabled to revive the comparison. Two related cost changes landed in the same effort: the Cloud NAT + router (`main.tf`) are now gated on `enable_gke` (they exist solely for Autopilot node egress — Cloud Run reaches Cloud SQL over private VPC peering and egresses public traffic via its own managed path), and the production Cloud SQL instance drops from `REGIONAL` HA to `ZONAL` (`var.db_availability_type`, default `ZONAL`) since a single-user app does not need cross-zone failover. See #860 for the full analysis.
