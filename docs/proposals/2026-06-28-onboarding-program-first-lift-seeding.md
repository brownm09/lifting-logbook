...
3. **Seed the lifts panel from the selected program, only when empty.** When the user advances out of the new program step, derive the program's canonical lift list and, **if and only if the current `lifts` state is empty (or still the untouched default)**, replace it with rows for those lifts (`{ lift, weight: '', reps: '' }`). If the user has already added their own lifts, leave their list untouched. `addLift` already de-duplicates by lift name, so seeding can reuse the same merge discipline. The seed must preserve the program's lift order.
...
4. **Choose a single lift source and document the choice.** Two sources exist:
   ...
   The brief specifies seeding from `PRESET_BASE_SPECS`. That is the right authoritative source, but it means RPT gets no seed until #592 lands. The wizard must degrade gracefully: when the selected program has no entry in `PRESET_BASE_SPECS`, the lifts panel falls back to today's behaviour (the existing `DEFAULT_LIFTS`, or an empty list) and onboarding still completes normally. Whether to *also* fall back to `PROGRAMS[].lifts` for unmapped programs is an open question (see below) — the safe default is to ship the empty/`DEFAULT_LIFTS` fallback and let #592 close the RPT gap.
...
- [ ] Selecting a program whose ID is a key in `PRESET_BASE_SPECS` (`5-3-1`, `leangains`) seeds the Enter Lifts panel with that program's canonical lifts, deduplicated and in spec order, **only when the lifts list is currently empty / still the untouched default**.
- [ ] If the user has already added one or more lifts manually before the program is (re)selected, their list is **not** overwritten.
- [ ] Selecting a program with **no** `PRESET_BASE_SPECS` entry (e.g. `rpt` until #592) does not error: the lifts panel shows the existing fallback and onboarding completes normally.
...
- Does the `DEFAULT_LIFTS` constant still have a role once program-first seeding exists, or should it become purely the unmapped-program fallback?
