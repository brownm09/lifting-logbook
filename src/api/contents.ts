...
export function updateTOC() {
  ...
  try {
    ...
    emptySheet(tocSheet);
    tocSheet
      .getRange("A1:A" + idNameTuples.length + 1)
...
export function arrangeCellData(sheetLinks) {
  ...
  const columnDataOther = sheetLinks.filter(
    (data) => !(columnData531.includes(data) || columnDataRpt.includes(data)),
  );
  Logger.log(
    `[DEBUG] Number of sheets per category: 5/3/1 (${columnData531.length}), RPT (${columnDataRpt.length}), Other (${columnDataOther.length})`,
  );
  for (
    var i = 0;
    i < columnData531.length ||
    i < columnDataRpt.length ||
    i < columnDataOther.length;
    i++
  ) {
    data.push([
