...
function updateRptNumbers(sheet) {
  ...
  setRepSchemes.forEach((range) => {
    ...
    for (var tempOffset = 1; tempOffset + rowIdx < dataRange.getLastRow() && repsOffset === -1; tempOffset++) {
      var firstColStr = sheet.getRange(rowIdx + tempOffset, nameColumnIdx).getValue().toString();
      if (firstColStr === "Set 1") {
        Logger.log(`[DEBUG] Found top set for "${lift}" (row ${rowIdx + tempOffset}).`);
        repsOffset = tempOffset;
      }
      if (firstColStr === "Notes") {
        Logger.log(`[DEBUG] Did not find "Set 1" before reaching "Notes" for "${lift}" (row ${rowIdx + tempOffset}).`);
        break;
      }
    }
    if (repsOffset === -1) {
      Logger.log(`[ERROR] Could not find "Set 1"; cannot update training maxes for "${lift}"`);
    } else {
      const topSetWeight = parseFloat(sheet.getRange(rowIdx + repsOffset, lastColumnIdx - 1).getValue());
      const topSetReps = parseInt(sheet.getRange(rowIdx + repsOffset, lastColumnIdx).getValue());
      if (date.toString().length) {
        const entryDate = new Date(date);
        // updateRecordsRpt(entryDate, lift, topSetWeight, topSetReps);
      } else {
        Logger.log(`[ERROR] Could not find date; cannot record training maxes for "${lift}"`);
      }
...
function updateRecordsRpt(date, lift, weight, reps) {
  ...
  var historySheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(RPT_HISTORY_SHEET_TITLE);
...
function updateTrainingMaxRpt(sheet, lift) {
  ...
  const rptLiftRange = sheet.getRange(rangeStartRow, 1, rangeStartRow + 10, sheet.getLastColumn());
  ...
  const bumpCol = rptLiftRange.createTextFinder("Inc. Amt.").findNext().getColumn();
  ...
  origTmCell.setValue(`=MROUND(${onDiet(sheet) ? absoluteBump : percentBump}, ${roundAndBumpAmt})`);
}
...
function nextDate(dayIndex, startDate = new Date()) {
  ...
  var daysSinceEpochStartDate = parseInt(startDate.getTime()/8.64e7);
  var daysSinceEpochCurrDate = parseInt(today.getTime()/8.64e7);
  ...
  Logger.log(`Start date ${startDate.toUTCString()}; index: ${startDate.getDay()}; dayIndex: ${dayIndex}`)
