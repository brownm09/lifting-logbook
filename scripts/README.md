...
| [`inject-otel-sidecar.py`](inject-otel-sidecar.py) | Builds the two-container Cloud Run manifest for the api service by deriving from the live service (`gcloud run services describe --format=export`): adds the `otel-collector` sidecar + config-secret volume, sets the api's `OTEL_EXPORTER_OTLP_ENDPOINT`, and drops the unused `SYSTEM_DATABASE_URL`. The `deploy.yml` api-deploy steps pipe its output to `gcloud run services replace` (#768). Idempotent; requires PyYAML. |
...
| [`test_inject_otel_sidecar.py`](test_inject_otel_sidecar.py) | Stdlib-`unittest` harness for [`inject-otel-sidecar.py`](inject-otel-sidecar.py) (#786): fixture-driven checks that the injector produces a 2-container (`api` + `otel-collector`) manifest, is idempotent, drops `SYSTEM_DATABASE_URL`, sets `OTEL_EXPORTER_OTLP_ENDPOINT` once, keeps `ports:` only on the api container, and fails non-zero on a malformed manifest / missing env var. Fixture: [`testdata/cloud-run-api-export.yaml`](testdata/cloud-run-api-export.yaml). Runs in CI (`ci.yml` → `lint-and-test`); run locally with `python scripts/test_inject_otel_sidecar.py -v` (needs `pip install pyyaml`). It is **not** part of `npm test` — the repo has no Python test runner, so this is invoked directly. |
