...
function validateNamedRanges(sheet) {
  ...
  Logger.log(`Validating named ranges for sheet: ${currSheetName}, ID: ${currSheetId}`);
  var namedRanges = sheet.getNamedRanges().filter((r) =>
    `${r.getRange().getSheet().getSheetId()}`.valueOf() === currSheetId.valueOf()
  );
  Logger.log(`# of Named ranges associated with current sheet: ${namedRanges.length}`);
  namedRanges.forEach((range) => {
    ...
    if (rangeName !== expectedRangeName) {
      Logger.log(`Name mismatch - Expected: ${expectedRangeName}, Actual: ${rangeName}`);
...
function deleteNamedRanges(sheet) {
  ...
  var namedRanges = sheet.getNamedRanges()
    .filter((r) => {
      ...
      Logger.log(`Removing named range ${r.getName()} (${r.getRange().getA1Notation()})`);
      r.remove();
    });
}
