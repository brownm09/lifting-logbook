...
function cleanupAllPreviousYearSheets() {
  ...
  Logger.log('--- Starting Global Cleanup for all years prior to ' + currentYear + ' ---');
  ...
  if (allOldSheets.length > 0) {
    ...
    for (var i = 0; i < allOldSheets.length - 1; i++) {
      ...
      try {
        ...
        Logger.log('ERROR: Could not delete ' + target.name + '. Error: ' + e.toString());
      }
    }
    ...
    if (!dryRun) {
      ...
      Logger.log('DRY RUN: Would have renamed ' + keepSheetObj.name + ' to ' + newName);
    }

  } else {
    Logger.log('No sheets found matching the pattern for any year prior to ' + currentYear + '.');
  }
