function recreateLiftHistory() {
  ...
  histSheet.getRange(1, 1, 1, RPT_HISTORY_HEADERS.length).setValues([RPT_HISTORY_HEADERS]);
  ...
  cycleSheetList.forEach((cycleSheet) => {
    console.log(`Adding history from ${cycleSheet.getName()} to ${histSheet.getName()}`)
...
function updateRptHistory(cycleSheet, historySheet) {
  ...
  const setRepSchemes = dataRange.createTextFinder(setRepSchemeRegex).useRegularExpression(true).startFrom(cycleSheet.getRange(1,1)).findAll();
  const setRepSchemeIdxs = setRepSchemes.map((range) => { return range.getRow() - 1; });
  if (setRepSchemeIdxs.length) {
    const sheetData = cycleSheet.getDataRange().getValues();
    const liftData = findWorkingSetReps(sheetData, 
      0,
      1, 
      2, 
      3, 
    ...
    liftData.forEach((row) => {
      ...
      appendLiftRecord(historySheet, "RPT", cycleNum, ...row);
    });
  }
}
 ...
 */
function findWorkingSetReps(dataValues, nameColIdx, specColIdx, repsColIdx, noteColIdx, rowIdxs) {
  var liftData = [], targetSetNum = 1, rowIdx, liftSpec, liftName,  setNum,  liftDate, setNotes, setReps,  setWeight, setNameMatches, weightMatches, repMatches, setsFound, workoutDates = [], workoutNum = 0;
  ...
  do {
    ...
    for (var i = rowIdx; i < dataValues.length; i++) {
      if (setNameMatches = `${dataValues[i][nameColIdx]}`.match(SET_NAME_REGEX)) {
        if (!setsFound) {
          setsFound = true;
        }
        weightMatches = `${dataValues[i][specColIdx]}`.trim().match("^([0-9\.]+)");
        ...
        if (weightMatches && weightMatches.length > 1) {
          ...
          if (repMatches && repMatches.length > 1) {
            ...
            liftData.push([workoutNum, liftDate, liftName, setNum, setWeight, setReps, setNotes]);
          }
        }
      } else {
        if (setsFound) {
 ...
 */
function appendLiftRecord(historySheet, progName, cycleNum, workoutNum, date, lift, setNum, weight, reps, notes) {
  const ROW_VALUES = [progName, cycleNum, workoutNum, date, lift, setNum, weight, reps, notes];
